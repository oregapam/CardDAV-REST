...
def vcard_to_contact(vcf: str, name_format: str = NAME_FORMAT_DEFAULT) -> ContactOut:
    ...
    return ContactOut(
        ...
        prefix=prefix,
        suffix=suffix,
        emails=[TypedValue(type=_type_of(el), value=el.value) for el in c.get("email", []) if el.value and el.value.strip()],
        phones=[TypedValue(type=_type_of(el), value=el.value) for el in c.get("tel", []) if el.value and el.value.strip()],
        addresses=addresses,
        org=org,
        title=text("title"),
        birthday=text("bday"),
